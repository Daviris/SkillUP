...
<div class="max-w-4xl mx-auto">
    <!-- Navegación -->
    <div class="mb-4">
        <a href="/mis-cursos/ver/<?= $curso['id'] ?>" class="text-amber-400 hover:text-amber-300 transition">
            ← Volver al curso
        </a>
    </div>
    ...
    <div class="bg-gray-800 border-2 border-amber-700 rounded-lg shadow-xl overflow-hidden">
        ...
        <div class="bg-amber-900/50 px-6 py-4 flex justify-between items-center">
            <div>
                <span class="text-sm text-amber-400">Clase <?= $clase['orden'] ?></span>
                <h1 class="text-2xl font-bold text-amber-300" style="font-family: 'VT323', monospace;">
        ...
        <div class="p-6">
                    ...
                    <div class="flex items-center justify-between bg-gray-700 p-4 rounded-lg border border-amber-600">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-gray-400 text-sm"><?= htmlspecialchars($archivo['nombre_original']) ?></p>
                            </div>
                        </div>
                        <a href="/archivo/descargar/<?= $archivo['id'] ?>"
            ...
            <?php elseif ($clase['tipo'] === 'tarea'): ?>
                ...
                <!-- Instrucciones -->
                <div class="bg-gray-700 p-4 rounded-lg border border-amber-600 mb-6">
                    <h2 class="text-xl font-semibold text-amber-400 mb-2">Instrucciones</h2>
                    <?php if (!empty($clase['criterios_evaluacion'])): ?>
                        <div class="text-gray-300"><?= nl2br(htmlspecialchars($clase['criterios_evaluacion'])) ?></div>
                    <?php else: ?>
                        <p class="text-gray-500 italic">No hay instrucciones detalladas.</p>
                    <?php endif; ?>
                            ...
                            <div class="mt-4 pt-3 border-t border-green-600">
                                ...
                                <?php if ($entrega['feedback_instructor']): ?>
                                    <p class="text-gray-300 mt-1 text-sm">
                                        <span class="text-amber-400">Feedback:</span>
                                        <?= htmlspecialchars($entrega['feedback_instructor']) ?>
                                    </p>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                            <!-- Opciones para modificar/eliminar -->
