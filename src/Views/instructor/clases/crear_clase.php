...
<div style="max-width:800px; margin:0 auto;">
    <div class="fade-in-up card" style="padding:2.5rem;">
        ...
        <div style="text-align:center; font-size:3rem; margin-bottom:0.5rem;">
            <?= ($accion === 'Crear') ? '<i class="fa-solid fa-scroll"></i>' : '<i class="fa-solid fa-pencil"></i>' ?>
        </div>
        ...
        <form method="POST" action="<?= $accion === 'Crear' ? '/instructor/clases/guardar' : '/instructor/clases/actualizar/' . ($clase['id'] ?? '') ?>" enctype="multipart/form-data" id="clase-form" novalidate>
            ...
            <div class="form-group">
                ...
                <select name="tipo" id="tipo-select" class="form-select">
                    <option value="teoria" <?= (isset($clase['tipo']) && $clase['tipo'] === 'teoria') ? 'selected' : '' ?>><i class="fa-solid fa-book"></i> Teoría (texto)</option>
                    <option value="archivo" <?= (isset($clase['tipo']) && $clase['tipo'] === 'archivo') ? 'selected' : '' ?>><i class="fa-solid fa-file"></i> Archivo (PDF, etc.)</option>
                    <option value="tarea" <?= (isset($clase['tipo']) && $clase['tipo'] === 'tarea') ? 'selected' : '' ?>><i class="fa-solid fa-file-pen"></i> Tarea (entrega de alumno)</option>
                </select>
            </div>
            ...
            <div id="campo-tarea" class="hidden">
                <div style="border:1px solid #334155; border-radius:0.5rem; padding:1.5rem; margin-top:1rem; background:#0f172a;">
                    <h3 style="color:#fbbf24; margin-bottom:1rem;"><i class="fa-solid fa-file-pen"></i> Configuración de la tarea</h3>
                    <div class="form-group">
            ...
            <div style="display:flex; gap:1rem; margin-top:2rem;">
                <button type="submit" class="btn btn-primary" style="flex:1; background:linear-gradient(135deg, #b45309, #d97706); border:none; padding:0.8rem;">
                    <i class="fa-solid fa-sword"></i> <?= htmlspecialchars($accion) ?>
                </button>
